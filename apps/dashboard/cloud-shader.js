/**
 * Animated WebGL cloud background for the landing hero.
 *
 * The GLSL below is the Aceternity UI cloud shader, kept verbatim. Only the
 * host is different: the original ships as a React component, and this
 * dashboard is plain HTML/CSS/JS, so the setup and teardown are rewritten as a
 * small vanilla initialiser rather than a hook. No framework needed — the
 * shader never cared what rendered the canvas.
 */
...
/**
 * Starts the shader on a canvas. Returns a handle with `setPalette` for theme
 * changes and `destroy` for teardown, or null when WebGL is unavailable —
 * callers fall back to a plain CSS gradient rather than showing a dead canvas.
 */
export function initCloudShader(canvas, options = {}) {
  ...
  const params = {
    ...
    cloudColor: options.cloudColor ?? "#fbf8f2",
    skyTopColor: options.skyTopColor ?? "#3876ba",
    skyBottomColor: options.skyBottomColor ?? "#8cbfe8",
  };
  ...
  const draw = (now) => {
    ...
    const cloud = parseHex(params.cloudColor);
    const skyTop = parseHex(params.skyTopColor);
    const skyBottom = parseHex(params.skyBottomColor);

    gl.uniform1f(loc.time, elapsed);
    gl.uniform1f(loc.count, Math.min(6, Math.max(1, params.count)));
    ...
    // A still frame is all a reduced-motion viewer should get; stop the loop
    // entirely rather than re-rendering an identical image 60 times a second.
